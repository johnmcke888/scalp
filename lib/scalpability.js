 ...
 * Returns null if no score data available
 */
export function calculateScoreCloseness(homeScore, awayScore, sport) {
  // No score data = return null (not a default value)
  ...
  // Tied game = maximum closeness
  if (scoreDiff === 0) return 100;
  
  // Blowout = minimum closeness
...
export function calculateScalpability(market) {
  ...
  // Disqualifiers - return special status
  ...
  // Pre-game: not live yet
  if (!event.live) {
    return {
      score: null,  // No score for pre-game
      rating: 'UPCOMING',
      reason: 'Game has not started',
      breakdown: {
        timeRemaining: 100,
        priceUncertainty: calculatePriceUncertainty(team1.price, team2.price),
        scoreCloseness: null,  // No score data yet
        scoreDiff: null,
        homeScore: null,
        awayScore: null,
        team1Price: team1.price,
        team2Price: team2.price,
        priceDiff: Math.abs(team1.price - team2.price),
      },
      isUpcoming: true,
    };
  }
  ...
  // If score is null (no data), use price uncertainty more heavily
  const hasScoreData = scoreCloseness !== null;

  // Adjust weights based on available data
  let timeScore, uncertaintyScore, closenessScore;

  if (hasScoreData) {
    // Full weights when we have all data
    timeScore = timeRemaining * 100 * 0.30;
    uncertaintyScore = priceUncertainty * 0.35;
    closenessScore = scoreCloseness * 0.35;
  } else {
    // No score data - weight time and uncertainty more
    timeScore = timeRemaining * 100 * 0.40;
    uncertaintyScore = priceUncertainty * 0.60;
    closenessScore = 0;
  }
  ...
  return {
    ...
    breakdown: {
      ...
      timeRemainingRaw: timeRemaining,  // 0-1 decimal
      ...
      // Score data
      scoreCloseness: scoreCloseness !== null ? Math.round(scoreCloseness) : null,
      homeScore: event.homeScore,
      awayScore: event.awayScore,
      scoreDiff: event.homeScore !== null && event.awayScore !== null 
        ? Math.abs(event.homeScore - event.awayScore) 
        : null,
