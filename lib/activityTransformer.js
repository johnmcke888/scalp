...
export function transformActivities(apiResponse) {
  ...
  const resolutionActivities = activities
    ...
    .map(activity => {
      ...
      // Calculate payout from resolution
      const beforeRealized = parseFloat(beforePos.realized?.value || '0');
      const afterRealized = parseFloat(afterPos.realized?.value || '0');
      const resolutionPayout = afterRealized - beforeRealized;
      
      // Extract metadata (title, team info)
      const marketMetadata = beforePos.marketMetadata || afterPos.marketMetadata || {};
      ...
      return {
        ...
        league: '', // Not available in resolution
        teamColor: null,
        side: resolution.side || 'RESOLUTION', // LONG, SHORT, or NEUTRAL
        intent: 'RESOLUTION',
        price: resolutionPayout > 0 ? 1.0 : 0.0, // $1 if won, $0 if lost
        shares: Math.abs(parseFloat(beforePos.netPosition || '0')),
        timestamp,
        timeISO: updateTime,
        realizedPnl: resolutionPayout,
        costBasis: null,
        originalPrice: null,
