...
export function transformActivities(apiResponse) {
  ...
  // Process ACTIVITY_TYPE_TRADE activities
  ...
  trades.push(...tradeActivities);

  // Process ACTIVITY_TYPE_POSITION_RESOLUTION activities  
  const resolutionActivities = activities
    .filter(activity => activity.type === 'ACTIVITY_TYPE_POSITION_RESOLUTION' && activity.positionResolution)
    .map(activity => {
      const resolution = activity.positionResolution;
      const beforePos = resolution.beforePosition || {};
      const afterPos = resolution.afterPosition || {};
      
      // Extract metadata and league info
      const marketMetadata = beforePos.marketMetadata || {};
      const team = marketMetadata.team || {};
      const league = (team.league || '').toUpperCase();
      
      // Parse position and financial data
      const shares = Math.abs(parseFloat(beforePos.netPosition || '0'));
      const costBasis = parseFloat(beforePos.cost?.value || '0');
      
      // FIXED: afterPos.realized is the P&L, not the payout
      // For correct payout calculation: payout = cost + P&L (but floor at zero)
      const realizedPnL = parseFloat(afterPos.realized?.value || '0');
      const calculatedPayout = costBasis + realizedPnL;
      const payout = Math.max(0, calculatedPayout); // Payout cannot be negative
      const profit = realizedPnL; // Use the actual P&L value
      
      // Determine win/loss - positive profit means win
      const won = profit > 0;
      
      // Parse side (POSITION_RESOLUTION_SIDE_LONG/SHORT)
      const rawSide = resolution.side || '';
      const side = rawSide.replace('POSITION_RESOLUTION_SIDE_', '');
      
      // Parse timestamp
      const updateTime = resolution.updateTime || '';
      const timestamp = updateTime ? new Date(updateTime).getTime() : Date.now();
      
      return {
        id: resolution.tradeId || `resolution-${timestamp}`,
        type: 'RESOLUTION', 
        marketSlug: resolution.marketSlug || '',
        title: marketMetadata.title || '',
        team: marketMetadata.outcome || '',
        league,
        teamColor: team.colorPrimary || null,
        side,
        intent: 'RESOLUTION',
        price: won ? 1.0 : 0.0, // $1 if won, $0 if lost
        shares,
        timestamp,
        timeISO: updateTime,
        realizedPnl: profit,
        costBasis,
        originalPrice: costBasis > 0 ? costBasis / shares : 0, // Average price paid
        isResolution: true, // Flag to identify resolution events
        
        // Additional resolution-specific fields
        payout,
        won,
        resolutionSide: side,
      };
    })
    .filter(resolution => resolution.marketSlug && resolution.shares > 0);

  trades.push(...resolutionActivities);
...
export function groupTradesByEvent(trades) {
  ...
  for (const trade of trades) {
    ...
    if (trade.type === 'RESOLUTION') {
      // For resolutions, just add the payout to realized P&L
      ...
      // Sum realized P&L from trades (if available)
      if (trade.realizedPnl !== null) {
        event.realizedPnl += trade.realizedPnl;
      }
    }
